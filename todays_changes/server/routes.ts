...
import { streamGroqChat, generateGroqSummary } from "./groq";
import { memoryService } from "./services/memory";
import { memoryBuffer } from "./services/memoryBuffer";
import { conversationTagger } from "./services/conversationTagger";
import { temporalAnalysisService } from "./services/temporalAnalysis";
import { relationshipDepthService } from "./services/relationshipDepth";
import { getAllPersonalities, getPersonalityById } from "./services/personality";
...
const FREE_CALL_DURATION_LIMIT = 1200; // 20 minutes in seconds
...
// Get user's total call duration from all call sessions
async function getUserCallDuration(userId: string): Promise<number> {
  const sessions = await storage.getUserSessions(userId);
  let totalDuration = 0;

  for (const session of sessions.filter((s) => s.type === "call")) {
...
async function hasExceededCallLimit(user: User): Promise<boolean> {
  ...
  const callDuration = await getUserCallDuration(user.id);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // PERSONALITY SYSTEM: Get all available personalities
  ...
  // PERSONALITY SYSTEM: Get user's current personality
  ...
  // PERSONALITY SYSTEM: Update user's personality
  app.post("/api/user/personality", async (req, res) => {
    try {
      ...
      // Update user profile
      await storage.updateUser(userId, { personalityModel: personalityId });
    ...
    try {
      ...
      if (messages.length === 0) {
        const greeting = await storage.createMessage({
          ...
          userId: session.userId,
          role: "ai",
          text: "Namaste! Main Riya hoon. It's so nice to meet you! Aap ka naam kya hai?",
        });
        res.json([greeting]);
      } else {
        res.json(messages);
  ...
  app.post("/api/chat", async (req, res) => {
    ...
    const userId = req.session.userId;
    console.log("[CHAT] User ID from session:", userId);

    if (!userId) {
      console.log("[CHAT] Authentication failed: No userId");
      res.status(401).json({ error: "Not authenticated" });
      return;
    }

    const { content, sessionId } = chatRequestSchema.parse(req.body);
    console.log("[CHAT] Parsed body:", { content, sessionId });

    const user = await storage.getUser(userId);
    if (!user) {
      console.log("[CHAT] User not found in DB:", userId);
      res.status(404).json({ error: "User not found" });
      return;
    }

    if (await hasExceededMessageLimit(user)) {
      console.log("[CHAT] User has exceeded message limit");
      const messageCount = await getUserMessageCount(user.id);
      res.json({
        limitExceeded: true,
        messageCount,
    ...
    // Get or create session and verify ownership
    let session: Session;
    if (sessionId) {
      ...
      if (existing && existing.userId === userId) {
        ...
        console.log("[CHAT] Creating new session (existing not found or unauthorized)");
      ...
      console.log("[CHAT] Creating new session (no sessionId provided)");
      session = await storage.createSession({
        userId: user.id,
        type: "chat",
      });
    ...
    // TIER 1.2: MEMORY BUFFER - Initialize buffer for this session if needed
    const existingBuffer = memoryBuffer.getBuffer(session.id);
    if (!existingBuffer) {
      memoryBuffer.initializeBuffer(session.id, user.id);
    }
    ...
    memoryBuffer.addMessage(session.id, "user", content);

    const allMessages = await storage.getSessionMessages(session.id);
    
    // Update memory buffer every 10 messages (extract summaries)
    const messageCount = memoryBuffer.getMessageCount(session.id);
    if (messageCount > 0 && messageCount % 10 === 0) {
      await memoryBuffer.updateMemoryBuffer(session.id);
    }

    // Limit context to last 50 messages to prevent token overflow
    ...
    console.log("[CHAT] Starting Groq stream...");
    console.log(
      "[CHAT] Conversation history length:",
    ...
    try {
      ...
      const relevantMemories = await memoryService.getRelevantMemories(user.id, content, 5);
      
      // Update last mentioned timestamps for memories that will be used
      for (const memory of relevantMemories) {
        await memoryService.updateMemoryLastMentioned(memory.id);
      }

      // Get working memory from buffer (summaries of current conversation)
      const workingMemory = memoryBuffer.getWorkingMemory(session.id);
      
      // Build context-aware system prompt with memories + working memory + temporal insights
      let contextAwarePrompt = await memoryService.buildContextAwareSystemPrompt(
        relevantMemories,
        user,
        RIYA_SYSTEM_PROMPT
      );
      ...
      // Fetch recent emotional state
      const recentEmotion = await storage.getRecentEmotionalState(user.id);

      // Add the new user message to history for the AI
      const fullHistory = [
        ...conversationHistory,
      ...
      await streamGroqChat(
        ...
        contextAwarePrompt, // Use context-aware prompt instead of base prompt
        recentEmotion ? {
          mood: recentEmotion.mood,
          energyLevel: recentEmotion.energyLevel ?? undefined,
          stressLevel: recentEmotion.stressLevel ?? undefined
        } : undefined
      );

      console.log(
        "[CHAT] Stream complete. Chunks:",
        chunkCount,
        "Total length:",
        fullResponse.length,
      );
      ...
      memoryBuffer.addMessage(session.id, "assistant", fullResponse);
      
      console.log("[CHAT] AI message created:", aiMessage.id);

      res.write(
        `data: ${JSON.stringify({ content: "", done: true, messageId: aiMessage.id, sessionId: session.id })}\n\n`,
      );
      res.end();

      // Trigger memory extraction asynchronously
      console.log("[CHAT] Triggering memory extraction...");
      memoryService.extractMemoriesFromSession(user.id, session.id)
        .catch(err => console.error("[CHAT] Memory extraction failed:", err));

      // Check for unlocks asynchronously
      (async () => {
        try {
          const messageCount = await getUserMessageCount(user.id);
          const unlockables = await storage.getUnlockableVisuals();
          const unlocked = await storage.getUnlockedContent(user.id);
          const unlockedIds = new Set(unlocked.map(u => u.contentId));

          for (const visual of unlockables) {
            if (unlockedIds.has(visual.id)) continue;

            if (visual.unlockCondition?.startsWith("message_count:")) {
              const requiredCount = parseInt(visual.unlockCondition.split(":")[1]);
              if (!isNaN(requiredCount) && messageCount >= requiredCount) {
                await storage.unlockContent({
                  userId: user.id,
                  contentId: visual.id,
                  unlockMethod: "milestone",
                });
                console.log(`[UNLOCK] User ${user.id} unlocked content ${visual.id}`);
              }
            }
          }
        } catch (err) {
          console.error("[CHAT] Unlock check failed:", err);
        }
      })();
      ...
      const errorMessage = groqError.message || "Failed to process message";
      const errorDetails = process.env.NODE_ENV !== "production" ? groqError.stack : undefined;

      res.write(
        `data: ${JSON.stringify({ error: errorMessage, details: errorDetails, done: true })}\n\n`,
      );
      res.end();
    }
  });
