import {
    ...
    type AdvancedMemory, type InsertAdvancedMemory,
    type MemoryEvolution, type InsertMemoryEvolution,
} from "@shared/schema";
import { randomUUID } from "crypto";
import { upsertRecord } from "./upsert-utility";

export interface IStorage {
    // Advanced Memory operations
    createAdvancedMemory(memory: InsertAdvancedMemory): Promise<AdvancedMemory>;
    getAdvancedMemories(userId: string): Promise<AdvancedMemory[]>;

    // Memory Evolution operations
    createMemoryEvolution(evolution: InsertMemoryEvolution): Promise<MemoryEvolution>;
    getMemoryEvolutions(memoryId: string): Promise<MemoryEvolution[]>;
...
export class MemStorage implements IStorage {
    ...
    private advancedMemories: Map<string, AdvancedMemory>;
    private memoryEvolutions: Map<string, MemoryEvolution>;
    ...
    constructor() {
        ...
        this.advancedMemories = new Map();
        this.memoryEvolutions = new Map();
    }

    // Advanced Memory operations
    async createAdvancedMemory(insertMemory: InsertAdvancedMemory): Promise<AdvancedMemory> {
        const id = randomUUID();
        const memory: AdvancedMemory = {
            id,
            ...insertMemory,
            sessionId: insertMemory.sessionId || null,
            referenceCount: 0,
            lastReferencedAt: null,
            userConfirmation: false,
            verificationStatus: "not_verified",
            clarificationNotes: null,
            rawTranscript: insertMemory.rawTranscript || null,
            createdAt: new Date(),
        };
        this.advancedMemories.set(id, memory);
        return memory;
    }

    async getAdvancedMemories(userId: string): Promise<AdvancedMemory[]> {
        return Array.from(this.advancedMemories.values())
            .filter((mem) => mem.userId === userId)
            .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    }

    // Memory Evolution operations
    async createMemoryEvolution(evolution: InsertMemoryEvolution): Promise<MemoryEvolution> {
        const id = randomUUID();
        const newEvolution: MemoryEvolution = {
            id,
            ...evolution,
            createdAt: new Date(),
        };
        this.memoryEvolutions.set(id, newEvolution);
        return newEvolution;
    }

    async getMemoryEvolutions(memoryId: string): Promise<MemoryEvolution[]> {
        return Array.from(this.memoryEvolutions.values())
            .filter((ev) => ev.memoryId === memoryId)
...
import { users, userSummaryLatest, sessions, messages, calls, subscriptions, payments, otpLogins, userMemories, engagementTriggers, userEmotionalStates, notificationTokens, avatarConfigurations, avatarInteractions, visualContent, unlockedContent, conversationTags, memoryTimeline, relationshipDepth, advancedMemories, memoryEvolutions } from "@shared/schema";
...
export class DbStorage implements IStorage {
    ...
    // Advanced Memory operations
    async createAdvancedMemory(insertMemory: InsertAdvancedMemory): Promise<AdvancedMemory> {
        const result = await db.insert(advancedMemories).values(insertMemory).returning();
        return result[0];
    }

    async getAdvancedMemories(userId: string): Promise<AdvancedMemory[]> {
        return await db.select()
            .from(advancedMemories)
            .where(eq(advancedMemories.userId, userId))
            .orderBy(desc(advancedMemories.createdAt));
    }

    // Memory Evolution operations
    async createMemoryEvolution(evolution: InsertMemoryEvolution): Promise<MemoryEvolution> {
        const result = await db.insert(memoryEvolutions).values(evolution).returning();
        return result[0];
    }

    async getMemoryEvolutions(memoryId: string): Promise<MemoryEvolution[]> {
        return await db.select()
            .from(memoryEvolutions)
            .where(eq(memoryEvolutions.memoryId, memoryId))
            .orderBy(desc(memoryEvolutions.createdAt));
    }
}
