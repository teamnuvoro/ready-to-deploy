...
export class RelationshipDepthService {
    ...
    /**
     * Get actual message counts from sessions
     */
    private async getActualMessageCounts(sessions: any[]): Promise<number> {
        if (sessions.length === 0) return 0;

        let totalMessages = 0;
        // In a real implementation, we would fetch message counts for each session
        // For now, we'll estimate or fetch if possible. 
        // Since we don't want to hammer the DB, we'll use a heuristic or average if not available

        // If sessions have a messageCount property (which they might in some queries)
        const withCounts = sessions.filter(s => typeof s.messageCount === 'number');
        if (withCounts.length > 0) {
            totalMessages = withCounts.reduce((sum, s) => sum + s.messageCount, 0);
            return totalMessages / withCounts.length;
        }

        // Fallback: assume average of 15 messages per session if not available
        return 15;
    }
