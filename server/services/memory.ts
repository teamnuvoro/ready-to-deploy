...
import { extractAdvancedMemories } from "./advancedMemory";

import { buildContextWithKnowledgeGraph } from "./memoryRelationships";
import { suggestMemoryUsage } from "./memorySuggestions";
...
Return ONLY a number between 0-100.`;
    ...
    async buildContextAwareSystemPrompt(
        ...
        const knowledgeGraphContext = await buildContextWithKnowledgeGraph(user.id);

        // TIER 5: Smart Suggestions (if current message exists)
        let suggestionContext = "";
        if (currentMessage) {
            const suggestions = await suggestMemoryUsage(user.id, currentMessage);
            if (suggestions.length > 0) {
                const top = suggestions[0];
                suggestionContext = `\nSUGGESTED MEMORY STRATEGY:\nReference: ${top.suggested_message_angle} (Confidence: ${top.confidence}%)`;
            }
        }

        if (userMemories.length === 0 && !knowledgeGraphContext) {
            return basePrompt;
        }
        ...
        // PERSONALITY SYSTEM REMOVED - Using Base Prompt
...
2. Show you remember details they shared.
3. Check progress on things they mentioned before.
4. Match their emotional energy.
5. Use the Knowledge Graph to understand their life holistically.
...
Use the memories naturally - like a real girlfriend who genuinely remembers and cares.`;
    ...
    async extractMemoriesFromSession(userId: string, sessionId: string) {
        try {
            ...
            // TIER 1: Advanced Memory Extraction
            console.log("[Memory] Starting Advanced Memory Extraction...");
            extractAdvancedMemories(userId, sessionId, transcript)
                .catch((err: any) => console.error("[Memory] Advanced extraction failed:", err));

            // 3. Call Groq for extraction (Basic)
