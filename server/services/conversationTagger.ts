...
${transcript.substring(0, 3000)}${transcript.length > 3000 ? '...' : ''}`;
    ...
    async tagConversation(sessionId: string, userMessage: string, aiResponse: string, userId: string): Promise<void> {
        try {
            const transcript = `User: ${userMessage}\nAI: ${aiResponse}`;
            const result = await this.autoTagConversation(sessionId, transcript);
            await this.saveConversationTags(userId, sessionId, result.tags, result.primaryEmotion, result.intensity);
        } catch (error) {
            console.error("[ConversationTagger] Error in tagConversation wrapper:", error);
        }
    }
}
